...
export function DashboardShell({ children, auth }: { children: React.ReactNode; auth: AuthContext }) {
  ...
  const setOpen = useUiStore((state) => state.setDashboardSidebarOpen);
  ...
  return (
    <div className="min-h-screen bg-background">
      ...
      <aside
        className={cn(
          "fixed inset-y-0 left-0 z-40 w-72 border-r border-border bg-slate-950/95 p-4 transition-transform lg:translate-x-0",
          open ? "translate-x-0" : "-translate-x-full lg:w-20"
        )}
      >
        <div className="flex items-center justify-between">
          <Link href="/dashboard" className={cn("text-2xl font-extrabold text-white", !open && "lg:hidden")}>
            <span className="gradient-text">MIDIGI</span>
          </Link>
      ...
      <main className={cn("min-h-screen px-4 py-6 transition-all sm:px-6 sm:py-8 lg:px-8", open ? "lg:ml-72" : "lg:ml-20")}>{children}</main>
    </div>
  );
}
